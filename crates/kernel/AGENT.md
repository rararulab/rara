...
- `JobWheel::trigger_now` returns an explicit three-way `TriggerOutcome { Fired, AlreadyInFlight, NotFound }` — NOT `Option<JobEntry>`. The syscall handler uses this to reply with `Result<TriggerJobReply, KernelError>` where the `Ok` arm is either `Fired` (publish `ScheduledTask`) or `AlreadyInFlight` (no dispatch, the prior agent will publish its own report) and the `Err` arm is reserved for `NotFound`. Do NOT collapse `AlreadyInFlight` into `NotFound` at the syscall boundary — the backend admin route relies on the distinction to return HTTP 200 with a `triggered: false` discriminator for a deduplicated click, instead of a misleading 404. Without this, spam-clicking "Run now" spawns N overlapping agent sessions (pre-f989f4e3 bug) or looks like a "not found" error (pre-fix regression).
