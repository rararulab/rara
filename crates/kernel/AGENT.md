...
- `JobWheel::trigger_now` is a no-op when the job is already in-flight. The syscall handler relies on `Some`/`None` to gate the `ScheduledTask` dispatch; without this dedupe, a spam-clicked "Run now" would spawn N overlapping agent sessions against the same job. The ledger itself dedupes by key, but the dispatch does not — the guard belongs at the wheel.
