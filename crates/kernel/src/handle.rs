...
impl KernelHandle {
    ...
    ///
    /// The per-session child concurrency limit is enforced inside
    /// `Kernel::handle_spawn_agent`, which acquires a permit from the
    /// parent's `child_semaphore` and stores it on the child session so it
    /// is released automatically when the child session is dropped. Spawns
    /// that exceed the limit fail fast with `KernelError::SpawnLimitReached`.
