...
impl Kernel {
    ...
    /// Set the lifecycle hook registry.
    ///
    /// Must be called before `start()`. The registry is shared with all
    /// agent sessions via the kernel handle.
    ...
    async fn handle_turn_completed(
        ...
        // Fire post_turn lifecycle hooks.
        {
            let model = self
                .process_table
                .with(&session_key, |p| {
            ...
            let turn_ctx = crate::lifecycle::TurnContext {
                session_key,
                user_text: String::new(), // not available here
                model,
            };
