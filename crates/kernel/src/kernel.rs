    ...
    async fn start_llm_turn(&self, session_key: SessionKey, msg: InboundMessage) {
        ...
        tokio::spawn(async move {
            ...
            let panic_result = std::panic::AssertUnwindSafe(async {
                ...
                // resolved execution mode. For `/plan <goal>`, strip the
                // command prefix and pass the goal text.
                let (effective_user_text, _is_plan) = if use_plan_executor {
                    let goal = user_text
                        .strip_prefix("/plan ")
                        .unwrap_or(&user_text)
                        .to_string();
                ...
                // TODO(plan-execute): When _is_plan is true, route to
                // plan_executor instead of run_agent_loop. The plan executor
                // will be implemented in plan.rs by another agent.
