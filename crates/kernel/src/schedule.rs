...
impl JobWheel {
    ...
    pub fn drain_expired_now(&mut self) -> Vec<JobEntry> {
...
mod tests {
    ...
    fn test_principal() -> Principal {
        Principal::from_user(&KernelUser {
            name:        "test".into(),
            role:        Role::Root,
    ...
    fn drain_uses_clock_time() {
        ...
        let drained = wheel.drain_expired_now();
        assert!(
            drained.is_empty(),
        ...
        let drained = wheel.drain_expired_now();
        assert_eq!(drained.len(), 1, "expected one job after advancing 61s");
    }
}
