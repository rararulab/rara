...
impl SessionService {
    ...
    ///
    /// The session key is generated by the repository (UUID).
    ...
    pub async fn ensure_session(
        ...
        match self.session_repo.get_session(key).await? {
            ...
            None => {
                ...
                Ok(self.session_repo.create_session(&entry).await?)
            }
        }
    }
    ...
    pub async fn export_session_to_memory(
        ...
        md.push_str(&format!("# Session: {key}\n\n"));
    ...
    pub async fn fork_session(
        ...
        let forked = self
            .session_repo
            .fork_session(source_key, fork_at_seq)
            .await?;
        info!(
            source = %source_key,
            target = %forked.key,
    ...
    pub async fn append_messages(
        ...
        if self.session_repo.get_session(key).await?.is_none() {
            ...
            self.session_repo.create_session(&entry).await?;
        }
