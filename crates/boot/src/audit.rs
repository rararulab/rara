...
//! SQLite-backed [`AuditLog`] implementation.
//!
...
impl AuditLog for PgAuditLog {
    async fn record(&self, event: AuditEvent) {
        ...
        tokio::spawn(async move {
            ...
            if let Err(e) = sqlx::query(
                "INSERT INTO kernel_audit_events (id, timestamp, agent_id, session_id, user_id, \
                 event_type, event_data, details) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            )
            .bind(id)
            .bind(timestamp)
    ...
    async fn query(&self, filter: AuditFilter) -> Vec<AuditEvent> {
        ...
        // SQLite uses positional `?` placeholders that bind sequentially.
