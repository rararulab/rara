...
impl crate::repository::ApplicationRepository for PgApplicationRepository {
    ...
    async fn find_all(
        ...
        if let Some(ref status) = filter.status {
            let store_status: db_models::ApplicationStatus = (*status).into();
        ...
        if let Some(ref channel) = filter.channel {
            let store_channel: db_models::ApplicationChannel = (*channel).into();
            let _ = write!(
                sql,
        ...
        if let Some(ref priority) = filter.priority {
            let store_priority: db_models::ApplicationPriority = (*priority).into();
            let _ = write!(
                sql,
...
mod tests {
    ...
    use crate::types::{ApplicationChannel, ChangeSource, Priority};
    use job_domain_core::id::{JobSourceId, ResumeId};
    ...
    fn job_uuid() -> uuid::Uuid {
        TEST_JOB_ID.parse().unwrap()
    }
    ...
    async fn update_application() {
        ...
        updated_app.submitted_at = Some(chrono::Utc::now());
    ...
    async fn find_all_with_status_filter() {
        ...
        app2.submitted_at = Some(chrono::Utc::now());
