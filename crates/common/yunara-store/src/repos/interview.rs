...
//! [`job_domain_interview::repository::InterviewPlanRepository`].

use std::fmt::Write;

use async_trait::async_trait;
use job_domain_core::id::{ApplicationId, InterviewId};
use job_domain_interview::{
...
#[async_trait]
impl job_domain_interview::repository::InterviewPlanRepository for PgInterviewPlanRepository {
    async fn save(&self, plan: &InterviewPlan) -> Result<InterviewPlan, InterviewError> {
        let store: models::interview::InterviewPlan = plan.clone().into();

        let row = sqlx::query_as::<_, models::interview::InterviewPlan>(
    ...
    async fn find_by_application(
        ...
        let rows = sqlx::query_as::<_, models::interview::InterviewPlan>(
    ...
    async fn find_all(
        ...
        if let Some(ref task_status) = filter.task_status {
            let store_status: models::interview::InterviewTaskStatus = (*task_status).into();
            let _ = write!(
                sql,
        ...
        let rows = sqlx::query_as::<_, models::interview::InterviewPlan>(&sql)
            .fetch_all(&self.pool)
            .await
            .map_err(map_err)?;
