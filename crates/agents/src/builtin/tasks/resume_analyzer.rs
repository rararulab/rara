...
//! the agent can invoke tools (e.g. access user skills) during analysis.
...
impl ResumeAnalyzerAgent {
    ...
    pub async fn analyze(&self, prompt: &str) -> Result<String, TaskAgentError> {
        ...
        match &self.tools {
            ...
            _ => run_completion(&*self.provider, &self.model, &system_prompt, prompt).await,
        }
    }
    ...
    async fn build_system_prompt(&self) -> String {
        let base = self
            .prompt_repo
            .get("ai/resume_analyzer.system.md")
            .await
            .map(|e| e.content)
            .unwrap_or_default();
        let soul = self
