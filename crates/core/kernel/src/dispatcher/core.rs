...
use super::{
    ...
    metrics,
    types::{
        AgentTask, DispatcherCommand, DispatcherStatus, PrioritizedTask, QueuedTaskInfo,
        RunningTaskInfo, RunningTaskInner, TaskRecord, TaskResult, TaskStatus,
    },
};
...
async fn handle_submit(
    ...
    if let Some(ref dedup_key) = task.dedup_key {
        let is_dup = {
            ...
            let running_dup = running_map
                .values()
                .any(|r| r.dedup_key.as_deref() == Some(dedup_key.as_str()));
    ...
    try_dispatch(running, queue, log_store, executor, finish_tx).await;
}
        ...
        // Register as running.
        {
            ...
            map.insert(
                ...
                RunningTaskInner {
                    ...
                    dedup_key: pt.task.dedup_key.clone(),
                },
            );
        }

        // Spawn execution via TaskExecutor.
        let executor = Arc::clone(executor);
        let log_store = Arc::clone(log_store);
        let finish_tx = finish_tx.clone();

        tokio::spawn(async move {
            let result = executor.execute(&pt.task).await;
