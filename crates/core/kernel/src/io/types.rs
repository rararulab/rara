...
//! outbound buses. Platform adapters convert raw events into [`InboundMessage`],
//! and the kernel publishes [`OutboundEnvelope`] for egress delivery.
...
/// converted into this type and published to the [`InboundBus`](super::bus::InboundBus).
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn ingest_error_display() {
        ...
        assert_eq!(
            err.to_string(),
            "identity resolution failed: no mapping"
        );
    }
