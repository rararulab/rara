    ...
    if let Some(ref tg_adapter) = telegram_adapter {
        // Wire command handlers now that KernelHandle is available.
        {
            use rara_channels::telegram::commands::{
                AdminCommandHandler, KernelBotServiceClient, SessionCommandHandler,
                StopCommandHandler,
            };
            let owner_chat_id = config.telegram.as_ref().and_then(|tg| tg.chat_id.clone());
            let gateway_bind_address = config.gateway.as_ref().map(|gw| gw.bind_address.clone());
            ...
            let handlers: Vec<std::sync::Arc<dyn rara_kernel::channel::command::CommandHandler>> = vec![
                std::sync::Arc::new(AdminCommandHandler::new(bot_client.clone(), owner_chat_id)),
