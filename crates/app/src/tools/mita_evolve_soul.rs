...
/// Input parameters for the evolve-soul tool.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct EvolveSoulParams {
    /// Target agent name (e.g. "rara").
    agent:         String,
    /// Full proposed soul.md content (YAML frontmatter + markdown body).
...
#[async_trait]
impl ToolExecute for EvolveSoulTool {
    type Output = Value;
    type Params = EvolveSoulParams;

    async fn run(&self, params: EvolveSoulParams, _context: &ToolContext) -> anyhow::Result<Value> {
        // 1. Load current soul.
        let loaded = rara_soul::load_soul(&params.agent)
            .map_err(|e| anyhow::anyhow!("failed to load soul file: {e}"))?
            .ok_or_else(|| anyhow::anyhow!("no soul file found for agent '{}'", params.agent))?;

        let current_soul = loaded.soul;
        let current_version = current_soul.frontmatter.version;

        // 2. Parse proposed soul.
        let mut proposed_soul = rara_soul::SoulFile::parse(&params.proposed_soul)
        ...
        let snapshots_dir = rara_soul::loader::snapshots_dir(&params.agent);
        ...
        let soul_file_path = rara_soul::soul_path(&params.agent);
        ...
        if let Ok(Some(mut state)) = rara_soul::loader::load_state(&params.agent) {
            ...
            let _ = rara_soul::loader::save_state(&params.agent, &state);
        }
        ...
        push_notification(
            _context,
            format!(
                "\u{1f9ec} Soul evolved: {} v{current_version} \u{2192} v{new_version}\n{}",
                params.agent, params.reason,
            ),
        );
