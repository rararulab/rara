...
mod tests {
    ...
    #[test]
    fn meta_string_extracts_present_value() {
        let meta: Meta = serde_json::from_value(json!({"connector_id": "abc123"})).unwrap();
        assert_eq!(
            RmcpClient::meta_string(Some(&meta), "connector_id"),
            Some("abc123".to_string()),
        );
    }

    #[test]
    fn meta_string_returns_none_for_missing_key() {
        let meta: Meta = serde_json::from_value(json!({"other": "value"})).unwrap();
        assert_eq!(RmcpClient::meta_string(Some(&meta), "connector_id"), None);
    }

    #[test]
    fn meta_string_returns_none_for_empty_value() {
        let meta: Meta = serde_json::from_value(json!({"connector_id": ""})).unwrap();
        assert_eq!(RmcpClient::meta_string(Some(&meta), "connector_id"), None);
    }

    #[test]
    fn meta_string_returns_none_for_whitespace_only() {
        let meta: Meta = serde_json::from_value(json!({"connector_id": "   "})).unwrap();
        assert_eq!(RmcpClient::meta_string(Some(&meta), "connector_id"), None);
    }

    #[test]
    fn meta_string_trims_surrounding_whitespace() {
        let meta: Meta = serde_json::from_value(json!({"connector_id": "  abc  "})).unwrap();
        assert_eq!(
            RmcpClient::meta_string(Some(&meta), "connector_id"),
            Some("abc".to_string()),
        );
    ...
    #[test]
    fn meta_string_returns_none_for_none_meta() {
        assert_eq!(RmcpClient::meta_string(None, "connector_id"), None);
    }

    #[test]
    fn meta_string_returns_none_for_non_string_value() {
        let meta: Meta = serde_json::from_value(json!({"connector_id": 42})).unwrap();
        assert_eq!(RmcpClient::meta_string(Some(&meta), "connector_id"), None);
    }
    ...
    #[test]
    fn is_no_auth_support_matches_correct_variant() {
        let err: anyhow::Error = AuthError::NoAuthorizationSupport.into();
        assert!(is_no_auth_support(&err));
    }

    #[test]
    fn is_no_auth_support_rejects_other_errors() {
