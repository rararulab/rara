...
- `src/web_session.rs` — **persistent per-session WS** at `GET /session/{session_key}` (#1935). Single ordered mpsc carries three event sources merged into one socket: adapter-local broadcast (`Typing`, `Error`, `Phase`, egress replies), kernel `StreamHub::subscribe_session_events` (in-turn `text_delta`, `tool_call_*`, `done`, …), and the kernel notification bus filtered to this session's `TapeAppended` (in-turn after `done` AND out-of-turn for background tasks / scheduled re-entries). Funnelling both `Done` and `TapeAppended` onto one ordered mpsc is what makes their wire order deterministic and kills the cross-WS race classes (#1601, #1731, #1849, #1877, #1923). Reuses `WebAdapterState` and the `web_reply_buffer` so reconnect within the TTL window replays "important" events. **Phase (a)** scope: scaffold + hello + drain + forwarders. Inbound `prompt` is rejected with an error frame; `abort` is parsed but only logged. Phase (b) lands full inbound handling.
...
- Do NOT route `tape_appended` over a separate WebSocket from in-turn stream events (`text_delta`, `done`, …) — **why:** ordering across two sockets is undefined, which produced the cross-WS race classes traced in #1601, #1731, #1849, #1877, #1923. The persistent endpoint in `web_session.rs` funnels both event sources through one ordered mpsc; preserve that property for any future event added to the union.
