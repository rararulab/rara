 ...
 *
 * Globally-shared bridge so vendored Island/dropdown layers can coordinate
 * dismissal (Esc, outside click) with the host app's layer manager. The host
 * may install a bridge via `setDismissibleLayerBridge`; layers query
 * `getDismissibleLayerBridge` and degrade gracefully when no bridge is set.
 */
