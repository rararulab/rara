use crate::{constraints::Constraint, glob_detect::has_wildcards};

/// Check if a token looks like a filename or file path for use as a `FilePath`
/// constraint.
///
...
/// - Last component (after final `/`) contains `.` with a valid-looking
///   extension (1–10 alphanumeric chars starting with a letter, e.g. `rs`,
///   `json`, `tsx`)
///
/// This covers both bare filenames (`score.rs`) and path-prefixed ones
/// (`src/main.rs`).
#[inline]
...
/// Parser configuration trait - allows different picker types to customize
/// parsing
pub trait ParserConfig {
    fn enable_glob(&self) -> bool { true }

    /// Should parse extension shortcuts (e.g., *.rs)
    fn enable_extension(&self) -> bool { true }
    ...
    fn enable_type_filter(&self) -> bool { true }

    /// Should parse git status (e.g., status:modified)
    fn enable_git_status(&self) -> bool { true }
    ...
    fn enable_location(&self) -> bool { true }
    ...
    fn is_glob_pattern(&self, token: &str) -> bool { has_wildcards(token) }

    /// Custom constraint parsers for picker-specific needs
    fn parse_custom<'a>(&self, _input: &'a str) -> Option<Constraint<'a>> { None }
}
...
impl ParserConfig for FileSearchConfig {
    /// Detect bare filenames (`score.rs`) and path-prefixed filenames
    /// (`src/main.rs`) as `FilePath` constraints so that multi-token
    /// queries like `score.rs file_picker` filter by filename first, then
    /// fuzzy-match the remaining text against the path.
...
impl ParserConfig for GrepConfig {
    ...
    fn enable_git_status(&self) -> bool { false }

    fn enable_location(&self) -> bool { false }
