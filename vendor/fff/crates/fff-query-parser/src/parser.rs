use crate::{
    ConstraintVec,
    config::ParserConfig,
    constraints::{Constraint, GitStatusFilter, TextPartsBuffer},
    glob_detect::has_wildcards,
    location::{Location, parse_location},
};
...
impl Default for QueryParser<crate::FileSearchConfig> {
    fn default() -> Self { Self::new(crate::FileSearchConfig) }
}

#[inline]
fn parse_token<'a, C: ParserConfig>(token: &'a str, config: &C) -> Option<Constraint<'a>> {
    // Backslash escape: \token → treat as literal text, skip all constraint
    // parsing. The leading \ is stripped by the caller when building the search
    // text.
...
/// Parse negation pattern: !*.rs -> Not(Extension("rs")), !test ->
/// Not(Text("test")) This allows negating any constraint type
#[inline]
...
/// Also supports multi-segment paths: libswscale/aarch64/ ->
/// PathSegment("libswscale/aarch64")
#[inline]
...
mod tests {
    ...
    fn test_grep_format_braces_not_glob() {
        let parser = QueryParser::new(GrepConfig);
        // Code like format!("{}\\path", var) must not have tokens eaten as glob
        // constraints. The trailing comma on the first token means both { } and
        // , are present, but the comma is outside the braces so it should NOT
        // trigger brace expansion.
    ...
    fn test_file_picker_single_token_filename_stays_fuzzy() {
        ...
        // return FFFQuery with Text fuzzy query so the caller uses it for fuzzy
        // matching.
