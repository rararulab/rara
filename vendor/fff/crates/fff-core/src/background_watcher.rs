...
use crate::{
    error::Error,
    file_picker::{FFFMode, FilePicker},
    git::GitStatusCache,
...
impl BackgroundWatcher {
    ...
    fn create_debouncer(
        ...
        // do not follow symlinks as then notifiers spawns a bunch of events for
        // symlinked files that could be git ignored, we have to property
        // differentiate those and if the file was edited through a
        ...
        // On Linux (inotify) RecursiveMode::Recursive creates one watch per
        // subdirectory including gitignored ones like node_modules/, which
        // wastes kernel resources. NonRecursive watches only the directories
        // that actually contain indexed files.
        //
...
fn handle_debounced_events(
    ...
    // this will be called very often, we have to minimiy the lock time for file
    // picker
    ...
    for debounced_event in &events {
        ...
        // When macOS FSEvents (or other backends) overflow their event buffer, the
        // kernel drops individual events and emits a Rescan flag telling us to
        // re-scan the subtree. Without handling this, modified source files can
        // be silently missed.
        ...
        for path in &debounced_event.event.paths {
            ...
            //   - macOS FSEvents uses Modify(Name(Any)) for both rename-in and rename-out,
            //     so we must stat the path to disambiguate.
            //
            // We cannot rely on event kind alone because:
            //   - Remove events are not always emitted (macOS often sends Modify(Name(Any))
            //     instead of Remove).
...
fn is_non_code_directory(path: &Path) -> bool { crate::ignore::is_non_code_directory(path) }
...
fn watch_git_status_paths(debouncer: &mut Debouncer, git_workdir: Option<&PathBuf>) {
    ...
    // index, index.lock, HEAD, packed-refs, MERGE_HEAD, CHERRY_PICK_HEAD,
    // REVERT_HEAD
