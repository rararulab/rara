...
/// 95 printable ASCII chars (32..=126) after lowercasing → ~70 distinct → 4900
/// possible. We cap at 5000 to cover all printable bigrams with margin.
...
impl BigramIndexBuilder {
    ...
    pub(crate) fn add_file_content(&self, skip_builder: &Self, file_idx: usize, content: &[u8]) {
        ...
        // Stack-local dedup bitsets: 1024 × u64 = 8 KB each, covers all 65536 bigrams
        // with margin have to fit in L1 cache
    ...
    pub fn is_ready(&self) -> bool { self.populated.load(Ordering::Relaxed) > 0 }
...
/// Copmressed into bitset for minimal usage, the layout of this struct actually
/// matters
#[derive(Debug)]
...
impl BigramFilter {
    ...
    pub fn set_skip_index(&mut self, skip: BigramFilter) { self.skip_index = Some(Box::new(skip)); }
    ...
    pub fn is_ready(&self) -> bool { self.populated > 0 }

    pub fn file_count(&self) -> usize { self.file_count }

    pub fn columns_used(&self) -> usize { self.dense_count }
    ...
    pub fn has_key(&self, key: u16) -> bool { self.lookup[key as usize] != NO_COLUMN }

    /// Raw lookup table (65536 entries mapping bigram key → column index).
    pub fn lookup(&self) -> &[u16] { &self.lookup }

    /// Flat dense bitset data at fixed stride `words`.
    pub fn dense_data(&self) -> &[u64] { &self.dense_data }
    ...
    pub fn dense_count(&self) -> usize { self.dense_count }

    /// Number of files that contributed content to the index.
    pub fn populated(&self) -> usize { self.populated }

    /// Reference to the optional skip-1 bigram sub-index.
    pub fn skip_index(&self) -> Option<&BigramFilter> { self.skip_index.as_deref() }
...
impl BigramOverlay {
    ...
    pub(crate) fn base_file_count(&self) -> usize { self.base_file_count }

    /// Get the tombstone bitset for clearing base candidates.
    pub(crate) fn tombstones(&self) -> &[u64] { &self.tombstones }

    /// Get all modified file indices (for conservative overlay merging when
    /// we can't extract precise bigrams, e.g. regex patterns).
    pub(crate) fn modified_indices(&self) -> Vec<usize> { self.modified.keys().copied().collect() }
}
