//! Core file picker: filesystem indexing, background watching, and fuzzy
//! search.
//!
...
//! 3. **Owns files**: Provides a values for search and provides a good entry
//!    point for fuzzy search and live grep
//!
//! # Lifecycle
//!
//! ```text
...
use crate::{
    background_watcher::BackgroundWatcher,
    bigram_filter::{BigramFilter, BigramIndexBuilder, BigramOverlay},
    error::Error,
    frecency::FrecencyTracker,
    git::GitStatusCache,
    grep::{GrepResult, GrepSearchOptions, grep_search},
    ignore::non_git_repo_overrides,
    query_tracker::QueryTracker,
    score::match_and_score_files,
    shared::{SharedFrecency, SharedPicker},
    types::{ContentCacheBudget, FileItem, PaginationArgs, ScoringContext, SearchResult},
};
...
impl FFFMode {
    pub fn is_ai(self) -> bool { self == FFFMode::Ai }
}
...
struct FileSync {
    ...
    ///
    /// `files[base_count..]` are overflow files added since the last full
    /// reindex. Deletions in the base use tombstones (`is_deleted = true`)
    /// to keep bigram indices stable.
    files:          Vec<FileItem>,
    /// Number of base files (the sorted prefix used for binary search /
    /// bigram).
    base_count:     usize,
...
impl FileSync {
    ...
    #[inline]
    fn files(&self) -> &[FileItem] { &self.files }

    /// Get the overflow portion (files added since last full reindex).
    #[inline]
    fn overflow_files(&self) -> &[FileItem] { &self.files[self.base_count..] }

    #[allow(dead_code)]
    fn get_file(&self, index: usize) -> Option<&FileItem> { self.files.get(index) }

    /// Get mutable file at index (works for both base and overflow)
    #[inline]
    fn get_file_mut(&mut self, index: usize) -> Option<&mut FileItem> { self.files.get_mut(index) }
...
impl FileItem {
    ...
    /// Create a FileItem using pre-fetched metadata to avoid a redundant stat
    /// syscall.
...
impl FilePicker {
    ...
    pub fn need_warmup_mmap_cache(&self) -> bool { self.warmup_mmap_cache }

    pub fn mode(&self) -> FFFMode { self.mode }

    pub fn cache_budget(&self) -> &ContentCacheBudget { &self.cache_budget }

    pub fn bigram_index(&self) -> Option<&BigramFilter> { self.sync_data.bigram_index.as_deref() }
    ...
    pub fn git_root(&self) -> Option<&Path> { self.sync_data.git_workdir.as_deref() }
    ...
    pub fn get_files(&self) -> &[FileItem] { self.sync_data.files() }

    pub fn get_overflow_files(&self) -> &[FileItem] { self.sync_data.overflow_files() }
    ...
    /// Always prefer new_with_shared_state for the consumer application, use
    /// this only if you know what you are doing. This won't spawn the
    /// backgraound watcher and won't walk the file tree.
    ...
    /// Update git statuses for files, using the provided shared frecency
    /// tracker.
    ...
    /// Add a file to the picker's files in sorted order (used by background
    /// watcher)
    ...
    /// Use this to prevent any substantial background threads from acquiring
    /// the locks
    ...
    /// Quick way to check if scan is going without acquiring a lock for
    /// [Self::get_scan_progress]
    pub fn is_scan_active(&self) -> bool { self.is_scanning.load(Ordering::Relaxed) }

    /// Return a clone of the scanning flag so callers can poll it without
    /// holding a lock on the picker.
    pub fn scan_signal(&self) -> Arc<AtomicBool> { Arc::clone(&self.is_scanning) }

    /// Return a clone of the watcher-ready flag so callers can poll it without
    /// holding a lock on the picker.
    pub fn watcher_signal(&self) -> Arc<AtomicBool> { Arc::clone(&self.watcher_ready) }
}
...
fn spawn_scan_and_watcher(
    ...
    std::thread::spawn(move || {
        ...
        if warmup_mmap_cache && !cancelled.load(Ordering::Acquire) {
            ...
            // Scale cache limits based on repo size (skip if caller provided an explicit
            // budget).
