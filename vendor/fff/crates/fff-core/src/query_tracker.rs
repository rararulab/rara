...
use crate::{db_healthcheck::DbHealthChecker, error::Error};
...
impl QueryTracker {
    ...
    /// Append a query to a history database within an existing write
    /// transaction.
    ...
    /// offset=0 returns most recent query, offset=1 returns 2nd most recent,
    /// etc.
    ...
    /// Only records query history (no file association tracking needed for
    /// grep).
    ...
    /// offset=0 returns most recent grep query, offset=1 returns 2nd most
    /// recent, etc.
