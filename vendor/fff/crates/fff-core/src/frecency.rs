...
use crate::{
    db_healthcheck::DbHealthChecker,
    error::{Error, Result},
    file_picker::FFFMode,
...
impl FrecencyTracker {
    ...
    pub fn db_path(&self) -> &Path { self.env.path() }
    ...
    /// Spawns a background thread to purge stale frecency entries and compact
    /// the database. Run it once in a while to purge old pages and keep DB
    /// file size reasonable.
    ///
    /// It's okay to not join this thread since it acquires locks for the db
    /// access
    ///
    /// ```
    /// use fff_search::{SharedFrecency, frecency::FrecencyTracker};
    ...
    /// Calculating modification score but only if the file is modified in the
    /// current git dir
...
mod tests {
    ...
    fn test_modification_score_interpolation() {
        ...
        // (time_offset = 12-1 = 11 hours, time_range = 24-1 = 23 hours, points_diff =
        // 4-2 = 2)
