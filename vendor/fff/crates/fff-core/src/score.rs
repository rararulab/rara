...
pub fn match_and_score_files<'a>(
    ...
    // Filename match detection: two tiers, cursor-based (no intermediate
    // bitset/Vec<bool>).
    // 1) Collect filenames only where match_end_col didn't land in the filename
    //    region.
    // 2) Batch SIMD on that subset, remap indices, sort for cursor walk in the
    //    scoring loop.
...
/// Check if a filename is a special entry point file that deserves bonus
/// scoring These are typically files that serve as module exports or entry
/// points
...
/// Score files by frecency when we have a filtered list (prefiltered by
/// constraints)
...
mod tests {
    ...
    fn test_partial_sort_with_same_scores() {
        ...
        // Should return all 5 items sorted: 200(9000), 200(1000), 100(8000), 100(5000),
        // 100(3000)
...
mod filename_bonus_tests {
    ...
    fn test_same_length_filename_no_false_exact() {
        ...
        // "item.rs" should NOT get exact_filename on "file.rs" even though stem lengths
        // match
        let files = vec![
...
mod multi_part_tests {
    ...
    fn test_single_path_matching() {
        let path = "core_workflow_service/kafka_event_consumer/src/ai_part_extraction_request/\
                    ai_part_extraction_request_handler.rs";
