...
//! - **Sparse-1** (gap=1): pairs across a single-byte wildcard, e.g. `a.b →
//!   (a,b)`
//!
...
#[inline]
fn bitset_or(a: &mut [u64], b: &[u64]) { a.iter_mut().zip(b.iter()).for_each(|(x, y)| *x |= *y); }

/// SIMD-friendly bitwise AND of two equal-length bitsets.
#[inline]
fn bitset_and(a: &mut [u64], b: &[u64]) { a.iter_mut().zip(b.iter()).for_each(|(x, y)| *x &= *y); }

impl BigramQuery {
    pub fn is_any(&self) -> bool { matches!(self, BigramQuery::Any) }
...
struct HirInfo {
    query:        BigramQuery,
    /// Possible first bytes (lowercased, printable ASCII) when this node
    /// matches.
...
/// we allow max_typs = min(len/3,2) every typo destroys at most 2 consecutive
/// bigrams So out of N bigrams at least N - 2 * max_typos have to present in
/// the matching fil
...
fn decompose_concat(parts: &[Hir]) -> HirInfo {
    ...
    // 3. Sparse-1 cross-boundary: across a single 1-byte-wide middle part. Catches
    //    `foo.bar` → sparse-1 `(o,b)` across the dot.
    if parts.len() >= 3 {
...
mod tests {
    ...
    ///   - `None`        → pure classes / unsupported syntax, Any is
    ///     acceptable.
    ///   - `Some(&[..])` → must be non-Any, and every listed bigram must
    ///     appear.
    #[test]
