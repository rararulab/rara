...
/// Skip zero or more modifier keywords (including `pub(crate)` style
/// visibility).
...
pub fn has_regex_metacharacters(text: &str) -> bool { regex::escape(text) != text }
...
pub struct GrepMatch {
    ...
    /// Can be used by the preview to seek directly without scanning from the
    /// top.
    pub byte_offset:        u64,
    ...
    /// Whether the matched line looks like a definition (struct, fn, class,
    /// etc.). Computed at match time so output formatters don't need to
    /// re-scan.
...
pub struct GrepResult<'a> {
    ...
    /// Total number of searchable files (after filtering out binary, too-large,
    /// etc.).
    pub filtered_file_count:  usize,
    /// Number of files that contained at least one match.
    pub files_with_matches:   usize,
    /// The file offset to pass for the next page. `0` if there are no more
    /// files. Callers should store this and pass it as `file_offset` in the
    /// next call.
...
#[inline]
const fn is_utf8_char_boundary(b: u8) -> bool { (b as i8) >= -0x40 }
...
/// - Returns `None` for invalid regex patterns — the caller falls back to
///   literal mode
...
fn char_indices_to_byte_offsets(line: &str, char_indices: &[usize]) -> SmallVec<[(u32, u32); 4]> {
    ...
    // Iterating all chars is O(n) in the line length which is bounded by
    // MAX_LINE_DISPLAY_LEN (512).
...
fn prepare_files_to_search<'a>(
    ...
    // On large repos (500k+ files) with no frecency data (fresh session,
    // benchmark), skipping the O(n log n) sort saves ~200ms per query.
...
///   4. Call `match_indices` only on passing lines to get character highlight
///      offsets
#[allow(clippy::too_many_arguments)]
fn fuzzy_grep_search<'a>(
    ...
    let matcher = neo_frizbee::Matcher::new(
        grep_text,
        &neo_frizbee::Config {
            // Use the real max_typos so frizbee's SIMD prefilter actually rejects non-matching
            // lines (~2 SIMD instructions per line vs full SW scoring).
...
pub fn grep_search<'a>(
    ...
    //   "name = *.rs someth" -> grep "name = someth" with constraint
    // Extension("rs")
    ...
    // groups, character classes, and sparse-1 bigrams across single-byte
    // wildcards).
    let bigram_candidates = if let Some(idx) = bigram_index
        && idx.is_ready()
    {
    ...
    let (files_to_search, filtered_file_count) = match bigram_candidates {
        Some(ref candidates) if constraints_from_query.is_empty() => {
            // this call is essentially free and much more efficient than allowing a
            // recollection
...
mod tests {
    ...
    fn test_multi_grep_search() {
        ...
        // Should find matches from file1 (GrepMode, GrepMatch) and file2
        // (PlainTextMatcher)
        assert!(
