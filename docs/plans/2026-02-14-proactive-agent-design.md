# Proactive Agent Design (Agent Soul)

## Overview

Proactive Agent 是一个定时后台 worker，它周期性地回顾用户最近的聊天活动，通过 LLM 判断是否需要主动发送 Telegram 消息。核心理念是让 AI 代理拥有可配置的"灵魂"（personality prompt），能在用户不主动提问时也能提供帮助。
...
| Component | Role |
|-----------|------|
| `ProactiveAgentWorker` | Cron-triggered `FallibleWorker<AppState>` 实现 |
| `AgentSettings` | 存储 soul prompt、enabled 开关、cron 表达式 |
| `SettingsSvc` | 提供运行时设置的热读取（每次 tick 重新读取 enabled 标志） |
...
```rust
// crates/domain/shared/src/settings/model.rs

pub struct AgentSettings {
    /// LLM 人格 prompt。None 使用内置默认。
    pub soul: Option<String>,
    /// 是否启用主动消息。支持运行时热切换。
    pub proactive_enabled: bool,
    /// Cron 表达式（5 字段格式）。修改后需重启服务生效。
    pub proactive_cron: Option<String>,
}
```

`AgentSettings` 作为 `Settings.agent` 字段嵌入，使用 `#[serde(default)]` 确保向后兼容旧的 KV 存储数据。
...
所有字段可选，遵循与 `ai` / `telegram` 相同的 partial update 语义。`soul` 和 `proactive_cron` 发送空字符串表示清空（恢复默认）。

## Worker Execution Flow

### Guard Checks (早期退出)

Worker 在每次 tick 执行以下检查，任一不满足则 `return Ok(())`：

1. **`agent.proactive_enabled`** — 运行时热读取，关闭后立即生效
...
4. 构建文本摘要，每条消息截断到 200 字符（UTF-8 安全）

### LLM Reflection

使用 `AgentRunner` 进行单轮 LLM 调用：

- **System prompt:** 用户自定义 soul（或内置默认）
- **User content:** 活动摘要 + 指令（"write a Telegram message or respond SKIP"）
- **Tools:** 空（纯文本生成，无 function calling）
- **Max iterations:** 1（单轮）
- **Model:** `settings.ai.model_for(ModelScenario::Chat)`

### Message Delivery

LLM 回复不是 "SKIP" 时，通过 `NotifyClient.send_telegram()` 入队：

```rust
SendTelegramNotificationRequest {
    chat_id:        Some(chat_id),      // from settings.telegram.chat_id
    subject:        Some("Proactive Agent"),
    body:           response_text,       // LLM 生成的消息
    priority:       Normal,
    max_retries:    3,
    reference_type: Some("proactive_agent"),
}
```

消息进入 PGMQ 队列后由 Telegram bot 消费循环异步发送。
...
Worker 在应用启动时从 `settings.agent.proactive_cron` 读取 cron 表达式，默认 `0 9,18,21 * * *`（每天 9:00、18:00、21:00）。

如果用户设置了无效的 cron 表达式，启动时会 **自动降级**到默认 cron 并输出 warn 日志，不会 panic。

### 热更新限制

| 设置项 | 热更新 | 说明 |
|--------|--------|------|
| `proactive_enabled` | 支持 | 每次 tick 检查 |
| `soul` | 支持 | 每次 tick 读取 |
| `proactive_cron` | 需重启 | Cron 表达式在启动时固定 |

## Frontend UI

Settings 页面新增 "Agent Personality" 卡片（位于 AI 和 Telegram 之间）：
...
| `crates/domain/shared/src/settings/router.rs` | `AgentSettingsView` + API 暴露 |
| `crates/workers/src/proactive.rs` | `ProactiveAgentWorker` 实现 |
| `crates/workers/src/lib.rs` | 模块导出 |
| `crates/workers/src/worker_state.rs` | `llm_provider` 添加到 `AppState` |
| `crates/app/src/lib.rs` | Worker 注册 + cron 验证 |
