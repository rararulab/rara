...
### 1.12 Git 协作习惯（Ryan 偏好）

* 在 worktree 分支准备提交 PR 前，必须先 `rebase` 到 **本地 `main`**（不是远端追踪分支）。
* 当分支历史因 rebase 发生重写时，使用 `git push --force-with-lease` 更新远端分支。
* 如果该分支已经存在 PR，rebase 后按 Ryan 的习惯：关闭旧 PR，并重新创建一个新的 PR（不要仅更新旧 PR）。
